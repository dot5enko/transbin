...
func NewDecodeBuffer(order binary.ByteOrder) *decode_buffer {
	...
	result.pop_buff.allocator = &buff_allocator{}
	result.InitStack()

	return result
}

func (this decode_buffer) BranchAt(pos int) decode_buffer{
	this.pos = pos
	return this
}
...
func (this *decode_buffer) PushState(data []byte, at int) {
	...
	this.states[this.statesPos] = BufferState{pos: this.pos, size: this.size, ref: this.allocator.data}
